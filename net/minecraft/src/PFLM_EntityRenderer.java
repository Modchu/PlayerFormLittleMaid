...
public class PFLM_EntityRenderer extends EntityRenderer {
	...
	private Class noBiomesXClass;
	private Class mod_ThirdPersonCamera;
	private Class mod_PFLM_PlayerFormLittleMaid;
	private boolean isnoBiomesX;

    public PFLM_EntityRenderer(Minecraft par1Minecraft)
    {
        super(par1Minecraft);
    	//System.out.println("EntityRendererPlayerFormLittleMaid");
        ...
        isnoBiomesX = (Boolean) Modchu_Reflect.getFieldObject(mod_PFLM_PlayerFormLittleMaid, "isnoBiomesX");
        if (isnoBiomesX) {
        	try {
        		noBiomesXClass = Modchu_Reflect.loadClass(getClassName("mod_noBiomesX"));
        	} catch (Exception e) {
        		e.printStackTrace();
        	}
        }
    ...
    private void setupCameraTransform(float par1, int par2)
    {
        ...
        if (isnoBiomesX
        		&& (Boolean) Modchu_Reflect.invokeMethod(noBiomesXClass, "isFinite")) {
    ...
    private void updateLightmap()
    {
        ...
        if (isnoBiomesX
        		&& (Boolean) Modchu_Reflect.getFieldObject(noBiomesXClass, "ClassicLight")) {
        ...
        for(int k = 0; k < 16; k++)
        {
            for(int l = 0; l < 16; l++)
            {
                ...
                if (isnoBiomesX) {
                	Field f11;
        			try {
        				f11 = noBiomesXClass.getField("LightTintRed");
        				Boolean ret;
        				try {
        					f2 = Float.valueOf((Integer) f11.get(null));
        				} catch (Exception e) {
        					e.printStackTrace();
        				}
        			} catch (Exception e1) {
        			...
        			try {
        				f11 = noBiomesXClass.getField("LightTintGreen");
        				Boolean ret;
        				try {
        					f3 = Float.valueOf((Integer) f11.get(null));
        				} catch (Exception e) {
        					e.printStackTrace();
        				}
        			} catch (Exception e1) {
        			...
        			try {
        				f11 = noBiomesXClass.getField("LightTintBlue");
        				Boolean ret;
        				try {
        					f4 = Float.valueOf((Integer) f11.get(null));
        				} catch (Exception e) {
        					e.printStackTrace();
        				}
        			} catch (Exception e1) {
    ...
    public void renderWorld(float par1, long par2)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            if (isnoBiomesX) GL11.glEnable(GL11.GL_ALPHA_TEST);
            ...
            if (isnoBiomesX) {
            	Method method;
    			try {
    				method = noBiomesXClass.getMethod("isFinite", ((Class) null));
    				Boolean ret = null;
    				try {
    					ret = (Boolean) method.invoke(null, ((Class) null));
    					//if (mod_noBiomesX.isFinite()){
    					if (ret){
    						//renderglobal.renderBounds(par1);
    						try {
    							Method method2 = renderglobal.getClass().getMethod("renderBounds", new Class[]{ Float.TYPE });
    							Object ret2; //–ß‚è’l
    							try {
    								ret2 = method2.invoke(renderglobal, new Object[]{ new Float(par1) });
    							} catch (Exception e) {
    								e.printStackTrace();
    							}
    						} catch (Exception e) {
            ...
            if (isnoBiomesX) {
            	...
            	Field f2;
    			try {
    				f = noBiomesXClass.getField("Generator");
    				f2 = noBiomesXClass.getField("GEN_BIOMELESS");
    				int ret;
    				int ret2;
    				try {
    					ret = (Integer) f.get(0);
    					try {
    						ret2 = (Integer) f2.get(0);
...
//renderSnow(par1);
    ...
    protected void renderSnow(float f)
    {
        if (isnoBiomesX) {
        	Field f1;
			try {
				f1 = noBiomesXClass.getField("SnowCovered");
				Boolean ret;
				try {
					ret = (Boolean) f1.get(null);
					//if(!mod_noBiomesX.SnowCovered || mc.thePlayer.dimension!=0) {
					if (ret || mc.thePlayer.dimension!=0) {
						return;
					}
				} catch (Exception e) {
					e.printStackTrace();
    ...
    private void updateFogColor(float par1)
    {
        ...
        if (mc.gameSettings.renderDistance < 2)
        {
        	...
        	if (isnoBiomesX) {
            	Field ff;
    			try {
    				ff = noBiomesXClass.getField("SunriseAtNorth");
    				Boolean ret;
    				try {
    					ret = (Boolean) ff.get(null);
