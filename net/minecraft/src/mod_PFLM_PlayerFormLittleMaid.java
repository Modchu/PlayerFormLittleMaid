...
public class mod_PFLM_PlayerFormLittleMaid extends BaseMod
{
	...
	public void load()
	{
		...
		Modchu_Reflect.setFieldObject(MMM_TextureManager, "defaultModel", new MultiModelBaseBiped[] {
					new MultiModel(0.0F),
					new MultiModel(0.1F),
					new MultiModel(0.5F)
			});
...
/*
		if (!isAether) {
			...
			//if (isSmartMoving) {
				...
				//map.put(EntityPlayerSP.class, var1);
...
/*
		...
		if (isOlddays) {
			if (!pflm_renderPlayer2.isInstance(RenderManager.instance.getEntityClassRenderObject(EntityPlayerSP.class))) {
		...
		//if(isSmartMoving
				//&& mc != null) PFLM_SmartMovingOther.onTickInGame(mc);
*/
...
/*
		if (isPlayerForm
					&& isSmartMoving) {
				if (!(RenderManager.instance.getEntityClassRenderObject(EntityPlayerSP.class) instanceof PFLM_RenderPlayerSmart)
...
/*
		...
		if (b)
		{
			...
			minecraft.renderGlobal = isAether ? new RenderGlobalAltPlayerFormLittleMaidAether(minecraft, minecraft.renderEngine) : new PFLM_RenderGlobalAlt(minecraft, minecraft.renderEngine);
...
/*
			int var3 = mc.thePlayer.dimension;
		        ...
		        mc.thePlayer = mc.playerController.createPlayer(mc.theWorld);
    ...
    public static void loadcfg() {
		...
		if (cfgdir.exists()) {
			if (!mainCfgfile.exists()) {
				...
				cfgMaxMinCheck();if(Physical_HurtSound.equalsIgnoreCase("false")) Physical_HurtSound = "random.hurt";
				String k[] = {
						"AlphaBlend",
...
*/
			MultiModelBaseBiped[] o = new MultiModelBaseBiped[3];
			o[0] = (MultiModelBaseBiped) o1[0];
			o[1] = (MultiModelBaseBiped) o1[1];
			o[2] = (MultiModelBaseBiped) o1[2];
...
*///110delete
			...
			try {
				...
				Modchu_Debug.Debug(test2 + " Check ok.");
				t = true;
				// b166deleteerpflmCheck = erpflmCheck == 3 ? 6 : 5 ;
			} catch (ClassNotFoundException e) {
			}
			if (t) {
//-@-b166
...
//@-@b166
	...
	public static Object[] modelNewInstance(String s) {
		//Modchu_Debug.mDebug("1modelNewInstance s="+s);
		...
		//Modchu_Debug.mDebug("2modelNewInstance s="+s+" c="+c);
		if (c != null) {
			//Modchu_Debug.mDebug("3modelNewInstance c != null s="+s+" c="+c);
			models = new Object[3];
			...
			if (o != null) {
				...
				if (isSmartMoving
					...
					models[1] = Modchu_Reflect.newInstance(c, new Class[]{ float.class, int.class, int.class }, new Object[]{ f1[0], 1, 0 });
					models[2] = Modchu_Reflect.newInstance(c, new Class[]{ float.class, int.class, int.class }, new Object[]{ f1[1], 1, 2 });
				...
				//Modchu_Debug.mDebug("6modelNewInstance ok end.");
				return models;
			}
		}
		//Modchu_Debug.mDebug("7modelNewInstance null end. s="+s);
		return null;
	}
	...
	public static float[] getArmorModelsSize(Object o) {
		return (float[]) Modchu_Reflect.invokeMethod(Modchu_ModelBiped.class, "getArmorModelsSize", o);
	}
