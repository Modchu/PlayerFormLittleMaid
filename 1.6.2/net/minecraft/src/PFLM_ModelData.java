...
//-@-152
	...
	private void setIsRiding(boolean b) {
		if (!((EntityLivingBase)owner instanceof EntityPlayer)) return;
...
//-@-152
	...
	private ItemStack getArmorItemInSlot(int i) {
		if (!((EntityLivingBase)owner instanceof EntityPlayer)) return null;
		return ((EntityPlayer)(EntityLivingBase)owner).inventory.armorItemInSlot(i);
	}

	private ItemStack getHeadMount() {
		if (!((EntityLivingBase)owner instanceof EntityPlayer)) return null;
		ItemStack itemStack = ((EntityPlayer)(EntityLivingBase)owner).inventory.getStackInSlot(9);
	...
	private EnumAction[] getActions() {
		...
		ItemStack litemstack = ((EntityPlayer)(EntityLivingBase)owner).inventory.getCurrentItem();
		lactions[getHandedness()] = litemstack != null && ((EntityPlayer)(EntityLivingBase)owner).getItemInUseCount() > 0 ? litemstack.getItemUseAction() : null;
		return lactions;
	}
	...
	private ItemStack[] getItems() {
		...
		itemStack[getHandedness()] = ((EntityPlayer)(EntityLivingBase)owner).inventory.getCurrentItem();
		return itemStack;
	}

	private boolean isPlanter() {
		if (!((EntityLivingBase)owner instanceof EntityPlayer)) return false;
		ItemStack itemStack = ((EntityPlayer)(EntityLivingBase)owner).inventory.getStackInSlot(9);
	...
	private boolean getIsSleeping() {
		...
		return ((EntityPlayer)(EntityLivingBase)owner).isPlayerSleeping()
				| sleeping;
	}

	private void setIsSleeping(boolean b) {
		if (!((EntityLivingBase)owner instanceof EntityPlayer)) {
		...
		if (sleeping != b) {
			sleeping = b;
			float f2;
			for (f2 = ((EntityPlayer)(EntityLivingBase)owner).rotationYaw; f2 < 0.0F; f2 += 360F) { }
			for (; f2 > 360F; f2 -= 360F) { }
    ...
    private boolean getIsLookSuger() {
    	if (!((EntityLivingBase)owner instanceof EntityPlayer)) return false;
    	ItemStack itemstack = ((EntityPlayer)(EntityLivingBase)owner).inventory.getCurrentItem();
    	if (itemstack != null) {
    		Item item = itemstack.getItem();
    ...
    private Object getInventory() {
    	if (!((EntityLivingBase)owner instanceof EntityPlayer)) return null;
    	return ((EntityPlayer)(EntityLivingBase)owner).inventory;
    }
