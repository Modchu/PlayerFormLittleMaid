...
public class PFLM_Config
{
	...
	public static void writerConfig(File file, String[] s) {
		...
		try {
			...
			bwriter.close();
			Modchu_Debug.mDebug("PFLM_Config "+ file.toString() +" new file create.");
		} catch (Exception e) {
			Modchu_Debug.Debug("PFLM_Config "+ file.toString() +" file writer fail.");
			e.printStackTrace();
		}
	}
		...
		if (list == null) {
			...
			try {
				...
				for (int i = 0; (rl = breader.readLine()) != null ; i++) {
					...
					if (rl.indexOf("showModel[]") != -1) {
						configList.add(rl);
						//Modchu_Debug.mDebug("PFLM_Config loadConfig "+ file.toString() +" load " + rl);
					}
				}
				cfgData.put(file, list);
				breader.close();
				//Modchu_Debug.mDebug("PFLM_Config loadConfig o = "+o.toString());
			} catch (Exception e) {
				Modchu_Debug.Debug("PFLM_Config loadConfig "+ file.toString() +" load fail.");
				e.printStackTrace();
			}
		} else {
			String s2;
			for (int i = 0; i < list.size() ; i++) {
				...
				if (s2.indexOf("showModel[]") != -1) {
					configList.add(s2);
					//Modchu_Debug.mDebug("PFLM_Config loadConfig "+ file.toString() +" load " + s2);
				}
			}
		}
	}
	...
	public static Object loadConfig(File file, String s, Object o) {
		...
		if (list == null) {
			...
			try {
				...
				cfgData.put(file, list);
				breader.close();
				//Modchu_Debug.mDebug("PFLM_Config loadConfig1 o = "+o.toString());
			} catch (Exception e) {
				Modchu_Debug.Debug("PFLM_Config loadConfig "+ file.toString() +" load fail.");
				e.printStackTrace();
			}
			...
			for (int i = 0; i < list.size() ; i++) {
				...
				if (s2.startsWith(s)) {
					int i1 = s2.indexOf('=');
					//if (s.equalsIgnoreCase("shortcutKeysTextureName[6]")) Modchu_Debug.mDebug("PFLM_Config loadConfig2 s2 = "+s2+" s="+s+" s.length()="+s.length()+" i1="+i1);
					if (i1 > -1
							&& s.length() == i1) {
						o = s2.substring(i1 + 1);
						break;
			...
			//Modchu_Debug.mDebug("PFLM_Config loadConfig2 o = "+o.toString());
		}
		return o;
	}
...
*/
	...
	public static void loadConfigPlayerLocalData(HashMap map, File file) {
		...
		try {
			...
			Modchu_Debug.Debug("PFLM_Config loadConfigShowModel load fail.");
			e.printStackTrace();
		}
		//Modchu_Debug.mDebug("PFLM_Config loadConfigOthersPlayer");
	}
